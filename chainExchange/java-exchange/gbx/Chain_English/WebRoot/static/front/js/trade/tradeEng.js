...
var trade = {
	...
	submitTradeBtcForm : function(tradeType, flag) {
		...
		if(fhasRealValidate == "" || fhasRealValidate =="false" || fhasRealValidate == false){
			util.showerrortips(errorele, "Please complete KYC Authentication first.<a href='/user/realCertification.html'>Go Identity.</a>");
			return;
		}
	...
	cancelEntrustBtc: function(id) {
        ...
        function(result) {
    		var fhasRealValidate = document.getElementById("fhasRealValidate").value;
    		if(fhasRealValidate == "" || fhasRealValidate =="false" || fhasRealValidate == false){
    			util.layerAlert("", "Please complete KYC Authentication first.<a href='/user/realCertification.html'>Go Identity.</a>", 4);
    			return;
    		}
    ...
    cancelAllEntrustBtc: function(id) {
        util.layerConfirm(language["trade.error.tips.10"],
                function(result) {
		    		var fhasRealValidate = document.getElementById("fhasRealValidate").value;
		    		if(fhasRealValidate == "" || fhasRealValidate =="false" || fhasRealValidate == false){
		    			util.layerAlert("", "Please submit the identity Authentication first.<a href='/user/realCertification.html'>Go to KYC</a>", 4);
		    			return;
		    		}
	...
	autoRefresh : function() {
		...
		$.get(url, function(data) {
			$.each(data.buys, function(key, value) {
				...
				var buyele = $("#buy" + key);
				if (buyele.length == 0) {
					$("#buybox").append('<li id="buy' + key + '"  class="list-group-item clearfix buysellmap" data-type="1" data-money="' + util.moneyformat(Number(value.price), coinCount1) + '" data-num="' + util.moneyformat(Number(value.amount), coinCount2) + '">' + '<span class="col-xs-2 greentips padding-clear" style="width:10%;">' + 'Buy' + (key + 1) + '</span>' + '<span class="col-xs-5 text-right padding-clear" style="width:20%;">' + util.moneyformat(Number(value.price), coinCount1) + '</span>' + '<span class="col-xs-5 greentips text-right padding-clear" style="width:35%;">' + util.moneyformat(Number(value.amount), coinCount2)+ '</span>' + '<span class="col-xs-5 greentips text-right padding-clear" style="width:35%;">' + util.moneyformat(Number(value.amount*value.price), coinCount2) + '</span></li>');
				} else {
			...
			$.each(data.sells, function(key, value) {
				...
				if (sellele.length == 0) {
					$("#sellbox").prepend('<li id="sell' + key + '"  class="list-group-item clearfix buysellmap" data-type="0" data-money="' + util.moneyformat(Number(value.price), coinCount1) + '" data-num="' + util.moneyformat(Number(value.amount), coinCount2) + '">' + '<span class="col-xs-2 redtips padding-clear" style="width:10%;">' + 'Sell' + (key + 1) + '</span>' + '<span class="col-xs-5 text-right padding-clear" style="width:20%;">' + util.moneyformat(Number(value.price), coinCount1) + '</span>' + '<span class="col-xs-5 redtips text-right padding-clear" style="width:35%;">' + util.moneyformat(Number(value.amount), coinCount2)+ '</span>' + '<span class="col-xs-5 redtips text-right padding-clear" style="width:35%;">' + util.moneyformat(Number(value.amount*value.price), coinCount2) + '</span></li>');
				} else {
			...
			if (p_new > trade.lastprice) {
				...
				$("#lastprice").parent().addClass("greentips");
			} else if (p_new < trade.lastprice) {
				$("#lastprice").html(p_new+" ↓");
				$("#lastprice").parent().addClass("redtips");
			}
