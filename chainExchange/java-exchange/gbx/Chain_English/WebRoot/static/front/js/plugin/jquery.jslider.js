...
(function($) {
	...
	var pluginName = 'slider', initAllSelector = '[data-role=slider], .slider', paramKeys = ['InitValue', 'Accuracy','Complete','Marker','Type','Markertop'];
	...
	$[pluginName] = function(element, options) {
		...
		marker,markertop,circle, // marker element
		...
		plugin.init = function() {
			...
			markertop = $('<div class="'+plugin.settings.markertop+'"></div>');
			markertopsplit = $('<div class="marker-top-plist"></div>');
			circle = $('<div class="circle"></div>');
			$points=$element.parent().find(".proportioncircle");
			complete.appendTo($element);
			marker.appendTo($element.parent());
			markertop.appendTo(marker);
			markertopsplit.appendTo(markertop);
