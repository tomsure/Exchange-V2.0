...
<h2 class="contentTitle">Edit Price Limit Details</h2>
					...
					<select type="combox" name="vid">
						<c:forEach items="${trademappings}" var="v">
							<c:if test="${v.fid == flimittrade.ftrademapping.fid}">
								<option value="${v.fid}" selected="true">${v.fvirtualcointypeByFvirtualcointype1.fShortName}-${v.fvirtualcointypeByFvirtualcointype2.fShortName}</option>
							</c:if>
							<c:if test="${v.fid != flimittrade.ftrademapping.fid}">
