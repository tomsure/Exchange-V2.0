...
public class FshoppinglogDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FshoppinglogDAO.class);
	// property constants
	...
	public Fshoppinglog findById(java.lang.Integer id) {
		...
		try {
			Fshoppinglog instance = (Fshoppinglog) getSession().get(
					"com.ruizton.main.model.Fshoppinglog", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fshoppinglog> findByExample(Fshoppinglog instance) {
		...
		try {
			List<Fshoppinglog> results = (List<Fshoppinglog>) getSession()
					.createCriteria("com.ruizton.main.model.Fshoppinglog")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fshoppinglog instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fshoppinglog as model where model."
					+ propertyName + "= ?";
	...
	public Fshoppinglog merge(Fshoppinglog detachedInstance) {
		...
		try {
			Fshoppinglog result = (Fshoppinglog) getSession().merge(
					detachedInstance);
