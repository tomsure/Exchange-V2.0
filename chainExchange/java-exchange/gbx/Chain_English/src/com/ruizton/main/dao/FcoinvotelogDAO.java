 ...
 * 
 * @see com.ruizton.main.model.Fcoinvotelog
...
public class FcoinvotelogDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FcoinvotelogDAO.class);
	...
	public Fcoinvotelog findById(java.lang.Integer id) {
		...
		try {
			Fcoinvotelog instance = (Fcoinvotelog) getSession().get(
					"com.ruizton.main.model.Fcoinvotelog", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fcoinvotelog> findByExample(Fcoinvotelog instance) {
		...
		try {
			List<Fcoinvotelog> results = (List<Fcoinvotelog>) getSession()
					.createCriteria("com.ruizton.main.model.Fcoinvotelog").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fcoinvotelog instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fcoinvotelog as model where model."
					+ propertyName + "= ?";
	...
	public Fcoinvotelog merge(Fcoinvotelog detachedInstance) {
		...
		try {
			Fcoinvotelog result = (Fcoinvotelog) getSession().merge(
					detachedInstance);
