...
public class FcapitaloperationDAO extends HibernateDaoSupport  {
	...
	public Map getTotalAmountIn(int type,String fstatus,boolean isToday) {
		...
		sql.append("select '人民币',IFNULL(sum(famount),0) totalAmount from Fcapitaloperation where fType="+type+" and fstatus in"+fstatus+" \n");
		if(isToday){
	...
	public List getTotalGroup(String filter) {
		Map map = new HashMap();
		StringBuffer sql = new StringBuffer();
		sql.append("select round(sum(famount),0) totalAmount,DATE_FORMAT(fLastUpdateTime,'%Y-%m-%d') fcreateTime from Fcapitaloperation \n");
		sql.append(filter);
