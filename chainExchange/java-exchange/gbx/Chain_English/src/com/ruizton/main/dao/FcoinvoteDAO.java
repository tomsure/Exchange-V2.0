 ...
 * 
 * @see com.ruizton.main.model.Fcoinvote
...
public class FcoinvoteDAO extends HibernateDaoSupport {
	...
	public Fcoinvote findById(java.lang.Integer id) {
		...
		try {
			Fcoinvote instance = (Fcoinvote) getSession().get("com.ruizton.main.model.Fcoinvote",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fcoinvote> findByExample(Fcoinvote instance) {
		...
		try {
			List<Fcoinvote> results = (List<Fcoinvote>) getSession()
					.createCriteria("com.ruizton.main.model.Fcoinvote").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fcoinvote instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Fcoinvote as model where model."
					+ propertyName + "= ?";
