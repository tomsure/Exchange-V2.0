...
public class FbalancetypeDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FbalancetypeDAO.class);
	// property constants
	...
	public Fbalancetype findById(java.lang.Integer id) {
		...
		try {
			Fbalancetype instance = (Fbalancetype) getSession().get(
					"com.ruizton.main.model.Fbalancetype", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fbalancetype> findByExample(Fbalancetype instance) {
		...
		try {
			List<Fbalancetype> results = (List<Fbalancetype>) getSession()
					.createCriteria("com.ruizton.main.model.Fbalancetype").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fbalancetype instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fbalancetype as model where model."
					+ propertyName + "= ?";
	...
	public Fbalancetype merge(Fbalancetype detachedInstance) {
		...
		try {
			Fbalancetype result = (Fbalancetype) getSession().merge(
					detachedInstance);
