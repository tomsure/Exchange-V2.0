...
public class FbalanceearningDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FbalanceearningDAO.class);
	// property constants
	...
	public Fbalanceearning findById(java.lang.Integer id) {
		...
		try {
			Fbalanceearning instance = (Fbalanceearning) getSession().get(
					"com.ruizton.main.model.Fbalanceearning", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fbalanceearning> findByExample(Fbalanceearning instance) {
		...
		try {
			List<Fbalanceearning> results = (List<Fbalanceearning>) getSession()
					.createCriteria("com.ruizton.main.model.Fbalanceearning")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fbalanceearning instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fbalanceearning as model where model."
					+ propertyName + "= ?";
	...
	public Fbalanceearning merge(Fbalanceearning detachedInstance) {
		...
		try {
			Fbalanceearning result = (Fbalanceearning) getSession().merge(
					detachedInstance);
